...
export default function Home() {
    ...
    useEffect(() => {
        fetch("https://localhost:5001/api/DateMaster")
    ...
    return (
        <div>
            ...
            <ul>
                ...
                <li><strong>Webpack development server</strong>. In development mode, there's no need to run a
                    production build. The development server (<code>npm start</code>) launches a browser window with the
                    application, which will automatically reload if you modify any files.
                </li>
            ...
            <p>The <code>ClientApp</code> subdirectory is a standard React application based on
                ...
                run <code>npm</code> commands such as <code>npm test</code> or <code>npm install
                </code>.</p>
            <p>Search: <input type="text" value={search} onChange={handleSearch}/></p>
            <p>{loading ? "Loading..." : ""}</p>
            <p>{error ? "Error" : ""}</p>
            <ul>
                {data.filter(item => item.name.toLowerCase().includes(search.toLowerCase())).map(item => <li
